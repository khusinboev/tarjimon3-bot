    ...
    try:
        if await CheckData.check_on_start(message.from_user.id):
            await message.answer(
                text="Xush kelibsiz\n\n\nWelcome to my bot", reply_markup=await LanguageInline.langs_inline(user_id))
        else:
            await message.answer(text="Botimizdan foydalanish uchun kanalimizga azo bo'ling"
                                      "\nSubscribe to our channel to use our bot",
                                 reply_markup=await LanguageInline.join_btn(user_id))
...
@router.callback_query(lambda query: query.message.text == "private", lambda message: message.chat.type == ChatType.PRIVATE)
async def check(call: CallbackQuery):
    ...
    try:
        if await CheckData.check_on_start(user_id):
            ...
            await call.message.answer(text="Choose languages", reply_markup=await LanguageInline.langs_inline(user_id))
        else:
